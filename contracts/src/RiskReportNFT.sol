...
contract RiskReportNFT is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    enum ReportType { VAULT_DEPOSIT, VAULT_REDEEM, SWAP }
    ...
    /// Maps keccak256(user, target) → tokenId.
    ...
    constructor() ERC721("PreFlight Risk Report", "PFRR") Ownable(msg.sender) {}
    ...
    function _buildSVG(RiskReport memory r) internal pure returns (string memory) {
        ...
        string memory preview = r.previewValue > 0
            ? string.concat("Preview: ", _shortNum(r.previewValue))
            : "Preview: N/A";
        ...
        return string.concat(
            ...
            '<stop offset="0%" stop-color="', riskColor, '" stop-opacity="0.9"/>',
            '<stop offset="100%" stop-color="', riskColor, '" stop-opacity="0.3"/>',
            '</linearGradient>',
            '</defs>',

            // Background
            '<rect width="360" height="220" rx="16" fill="url(#bg)"/>',
            '<rect width="360" height="220" rx="16" fill="none" stroke="', riskColor, '" stroke-width="1.5" stroke-opacity="0.4"/>',
            ...
            // Type badge
            '<rect x="250" y="14" width="95" height="22" rx="11" fill="', riskColor, '" fill-opacity="0.15" stroke="', riskColor, '" stroke-width="1"/>',
            '<text x="297" y="29" font-family="monospace" font-size="9" fill="', riskColor, '" text-anchor="middle">', typeLabel, '</text>',
            ...
            '<text x="16" y="98" font-family="monospace" font-size="22" font-weight="bold" fill="', riskColor, '">', riskLabel, '</text>',
            ...
            '<text x="16" y="153" font-family="monospace" font-size="10" fill="#c9d1d9">', _shortAddr(r.target), '</text>',

            // Preview + block
            '<text x="200" y="140" font-family="monospace" font-size="9" fill="#8b949e">BLOCK</text>',
            '<text x="200" y="153" font-family="monospace" font-size="10" fill="#c9d1d9">', r.blockNumber.toString(), '</text>',

            // Divider
            '<line x1="16" y1="165" x2="344" y2="165" stroke="#30363d" stroke-width="1"/>',

            // Preview value
            '<text x="16" y="182" font-family="monospace" font-size="9" fill="#8b949e">', preview, '</text>',

            // Status badge
            '<rect x="250" y="170" width="94" height="22" rx="11" fill="', statusColor, '" fill-opacity="0.15" stroke="', statusColor, '" stroke-width="1"/>',
            '<circle cx="265" cy="181" r="4" fill="', statusColor, '"/>',
            '<text x="296" y="185" font-family="monospace" font-size="9" fill="', statusColor, '" text-anchor="middle">', statusLabel, '</text>',

            // Amount
            '<text x="16" y="210" font-family="monospace" font-size="9" fill="#484f58">Amount: ', _shortNum(r.amount), ' | ', _shortAddr(r.user), '</text>',
    ...
    function _svgShield(string memory color) internal pure returns (string memory) {
        return string.concat(
            '<path d="M16 14 L24 11 L32 14 L32 22 C32 27 24 31 24 31 C24 31 16 27 16 22 Z" fill="', color, '" fill-opacity="0.2" stroke="', color, '" stroke-width="1.2"/>'
        );
    }
    ...
    function _buildFlagBar(uint8 crit, uint8 soft, uint8 total) internal pure returns (string memory) {
        ...
        string memory label = string.concat(
            uint256(crit).toString(), " critical  ",
            uint256(soft).toString(), " warnings  ",
            uint256(clean).toString(), " clean"
        );

        return string.concat(
            '<rect x="16" y="108" width="', barWidth.toString(), '" height="6" rx="3" fill="#21262d"/>',
            critW > 0 ? string.concat('<rect x="16" y="108" width="', critW.toString(), '" height="6" rx="3" fill="#f85149"/>') : "",
            softW > 0 ? string.concat('<rect x="', (16 + critW).toString(), '" y="108" width="', softW.toString(), '" height="6" fill="#d29922"/>') : "",
            cleanW > 0 ? string.concat('<rect x="', (16 + critW + softW).toString(), '" y="108" width="', cleanW.toString(), '" height="6" rx="3" fill="#3fb950"/>') : "",
            '<text x="16" y="126" font-family="monospace" font-size="8" fill="#8b949e">', label, '</text>'
        );
    }
    ...
    function _unpackFlags(uint32 packed, ReportType rt)
        internal
        pure
        returns (string memory out)
    {
        ...
        for (uint8 i = 0; i < count; ) {
            ...
            string memory entry = string.concat(
                '{"trait_type":"', name, '","value":"', flagSet ? "true" : "false", '"}'
            );
    ...
    function _attr(string memory key, string memory val, bool isString)
        internal
        pure
        returns (string memory)
    {
    ...
    function _riskStyle(RiskLevel level)
        internal
        pure
        returns (string memory color, string memory label)
    {
    ...
    function _statusStyle(Status s)
        internal
        pure
        returns (string memory color, string memory label)
    {
    ...
    function _key(address user, address target) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(user, target));
    }
