...
import {SwapV2GuardLib} from "../lib/SwapV2GuardLib.sol";
...
contract SwapV2Guard is Ownable, ReentrancyGuard, AutomationCompatibleInterface {
    ...
    /**
     * @notice constructor of  swap guard.
     ...
     */
    constructor(uint256 _snapshotBlockInterval, address _tokenGuard) {
    ...
    function validateSwapCheck(
        ...
        StoredSwapCheck memory currentCheck = SwapV2GuardLib.packedCheck(currentResult, amountOut);
        ...
        returns (SwapV2GuardResult memory)
    {
        ...
        storedUserChecksPerRouter[user][router] = SwapV2GuardLib.packedCheck(result, amountOut);
        ...
        returns (SwapV2GuardResult memory result, uint256 amountOut)
    {
        ...
        StoredSwapCheck memory storedCheck = storedUserChecksPerRouter[user][router];
        (result, amountOut) = SwapV2GuardLib.unpackedCheck(storedCheck);
