...
contract TokenGuard {
    ...
    function _checkERC20Interface(address token, TokenGuardResult memory r) private view {
        // decimals()
        try IERC20Min(token).decimals() returns (uint8 d) {
        ...
        // totalSupply()
        try IERC20Min(token).totalSupply() returns (uint256 s) {
        ...
        // symbol()
        try IERC20Min(token).symbol() returns (string memory sym) {
        ...
        // name()
        try IERC20Min(token).name() returns (string memory n) {
    ...
    function _checkRebasing(address token, TokenGuardResult memory r) private view {
        bool rebase =
            _selectorExists(token, 0x99a0c2b8) ||  // rebase()
    ...
    function _readSlot(address addr, bytes32 slot) private view returns (bytes32 value) {
        ...
        (bool ok, bytes memory data) = addr.staticcall(
            abi.encodeWithSignature("__storageSlotRead__()")
        );
     ...
     */
    function _staticReadSlot(address target, bytes32 slot)
        private
        view
        returns (bytes32 value)
    {
        if (slot == EIP1967_IMPL_SLOT) {
            ...
            (bool ok, bytes memory data) = target.staticcall(
                abi.encodeWithSelector(0x5c60da1b)
            );
        ...
        if (slot == EIP1822_PROXIABLE_SLOT) {
            ...
            (bool ok, bytes memory data) = target.staticcall(
                abi.encodeWithSelector(0x52d1902d)
            );
            if (ok && data.length >= 32) {
                assembly { value := mload(add(data, 32)) }
                return value;
     ...
     */
    function _selectorExists(address target, bytes4 selector)
        private
        view
        returns (bool found)
    {
        ...
        assembly {
            let ptr  := add(code, 32)
            let stop := add(ptr, sub(len, 3))
            for {} lt(ptr, stop) { ptr := add(ptr, 1) } {
