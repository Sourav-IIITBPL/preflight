...
contract LiquidityV2RouterTest is Test {
    ...
    function test_previewAndSetterFlowsWork() public {
        router.guardedPreview(address(ammRouter), address(tokenA), address(tokenB), 1e18, 2e18, LiquidityOpType.ADD);
        ...
        assertEq(uint8(guard.lastOperationType()), uint8(LiquidityOpType.ADD));
    ...
    function test_storeAndMintAddLiquidityCheckCallsPolicyAndNft() public {
        ...
        uint256 packed = router.storeAndMintAddLiquidityCheck(
            address(ammRouter), address(tokenA), address(tokenB), 4e18, 5e18, LiquidityOpType.ADD, hex"0102"
        );
    ...
    function test_guardedRemoveLiquidityTransfersLpAndOutputs() public {
        ...
        (uint256 amountAOut, uint256 amountBOut) = router.guardedRemoveLiquidity(params);
        vm.stopPrank();
    ...
    function test_guardedRemoveLiquidityEthWorks() public {
        ...
        (uint256 tokenOut, uint256 ethOut) = router.guardedRemoveLiquidityETH(params);
        vm.stopPrank();
